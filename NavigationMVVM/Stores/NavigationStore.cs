...
/*1 Вся навигация управляется через хранилище навигации NavigationStore в которой хранится текущая модель представления для приложения в централизованном состоянии (CurrentViewModel).
 Всякий раз когда текущая модель представления обновляется, мы вызываем событие CurrentViewModelChanged в котором текущая модель просмотра изменилась и наша основная модель просмотра прослушивает это событие*/